...
def show_report_search():
  global shelves
  data_read() # نقرأ البيانات أولاً لتكون محدثة دائماً
  
  ask = input("Enter the name of the product you want to search about :\n")
  
  # سنستخدم متغير علم (Flag) بسيط لنعرف هل وجدنا المنتج أم لا
  found = False 
  
  for ga , ba in shelves.items():
    # .lower() تجعل البحث لا يتأثر بكون الحروف كبيرة أو صغيرة (مثلاً Apple هي apple)
    if ask.lower() == ga.lower(): 
      found = True
      
      # نطبع رأس الجدول مرة واحدة فقط عند إيجاد المنتج
      ...
      type = ba["type"]
      if type == "lb":
        print(f"{ga}     :   {ba["quantity"]} Lb   :   {ba["price"]} $")
      elif type == "pic" :
        print(f"{ga}     :   {ba["quantity"]} pic   :   {ba["price"]} $")
      else:
        print(f"{ga}     :   {ba["quantity"]} kg   :   {ba["price"]} $")
        
  # إذا انتهت الحلقة ولم نجد المنتج، نخبر المستخدم بذلك بدلاً من الصمت
  if found == False:
    print("This product is not found in the store ❌")
